"""Generate k=5 SeqWalk pairs, post-select by energy, run legacy search, and plot."""
...
if __name__ == "__main__":
    ...
    legacy_output_plot = (
        output_dir / "seqwalk_7mer_on_off_target_hist_k5_legacy_selected.svg"
    )
    workbook_path = output_dir / "seqwalk_k5_post_selected_sequence_pairs.xlsx"
    ...
    write_sequence_pairs_sheet(workbook, f"k={K_VALUE}", sequence_pairs, on_energies)
    ...
    if selected_count == 0:
        ...
        if len(legacy_pairs) == 0:
            ...
            legacy_stats = sc.plot_on_off_target_histograms(
                ...
                show_plot=True,
                title="64 Post-Selected SeqWalk Pairs, No Flank",
                xlim=XLIM,
            )
    ...
    write_sequence_pairs_sheet(
        workbook,
        f"k={K_VALUE} post-selected",
        selected_pairs,
        selected_on_energies,
    )

    index_sheet = workbook.create_sheet(title="post-selected source IDs")
    index_sheet["A1"] = "Selected row IDs from the full k=5 SeqWalk sheet"
    ...
    write_sequence_pairs_sheet(
        workbook,
        f"k={K_VALUE} legacy-selected",
        legacy_pairs,
        legacy_on_energies,
    )

    legacy_index_sheet = workbook.create_sheet(title="legacy-selected source IDs")
    legacy_index_sheet["A1"] = "Selected row IDs from the full k=5 SeqWalk sheet"
