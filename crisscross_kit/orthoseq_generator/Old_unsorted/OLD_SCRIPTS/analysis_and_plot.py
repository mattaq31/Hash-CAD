...
def load_handles_and_antihandles(file_path):
    handles = []
    antihandles = []

    # Open file and read each line as a handle
    with open(file_path, 'r') as f:
        for line in f:
            handle = line.strip()  # Remove newline or extra whitespace
            handles.append(handle)
            antihandles.append(revcom(handle))  # Compute reverse complement and add to antihandles

    return handles, antihandles
...
# Path to the file containing sequences
    file_path = './all_remaining_sequences.txt'
    handles, antihandles = load_handles_and_antihandles(file_path)
    ...
    plt.axhline(y=max_ontarget_energy, color='black', linestyle='-', label='max allowed binding energy')
    plt.axhline(y=min_ontarget_energy, color='black', linestyle='-', label='min allowed binding energy')
    ...
    plt.title('Gibbs free energy distribution of complementary 7-mer sequences ')
   ...
   #     pickle.dump(stat_dict, f)
    ...
    plt.axhline(y=max_ontarget_energy, color='black', linestyle='-', label='cutoff on-target binding energy')
    plt.axhline(y=min_extreme_offtarget, color='g', linestyle='-', label='cutoff off-target binding energy')
    ...
    plt.xlabel('arbitrary index')
    plt.ylabel('Gibbs free energy (kcal/mol)')
    plt.title('handle Gibbs free energy off-targe vs on-target')
