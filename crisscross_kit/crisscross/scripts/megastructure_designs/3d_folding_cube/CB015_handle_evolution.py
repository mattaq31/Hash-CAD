if __name__ == '__main__':
    ...
    for s1 in range(1, 14 + 1):
        for s2 in range(15, 28 + 1):
            t1_pos = megastructure.slats[get_slat_key(2, s1)].slat_position_to_coordinate[s2 + 3]
            t2_pos = megastructure.slats[get_slat_key(2, s2)].slat_position_to_coordinate[32 - s1]
            linked_handles[(2, 'top', t1_pos)] = (2, 'bottom', t2_pos)  # layer, slat, handle index
    ...
    evolve_manager = EvolveManager(megastructure,
                                   ...
                                   log_tracking_directory='/Users/matt/Desktop/CB015_final_designs')
