...
output_folder_prefix = '/Users/stellawang/Documents/gitrepos/Crisscross-Design/crisscross_kit/crisscross/scripts/stella/SW160_SW161_repeatingunits_UPonly'
...
RU_name = "up"
amount_to_make = (150, 3) # target volume, number of copies
...
# Group by slats
UP_slat_IDs = [(1, x) for x in range(1,16+1)] # blue
...
for n_copy in range(amount_to_make[1]): 
    for n_slatnumber, (layer, id) in enumerate(UP_slat_IDs):
        old_slat_key = "layer{}-slat{}".format(layer, id)
        new_slat_key = "layer{}-slat{}-{}-copy{}".format(layer, id, RU_name, n_copy+1) 
        ...
        plate_well_mapping[new_slat_key] = (1, plate96[n_copy*16+n_slatnumber])
...
print("Writing SW160/SW161 echo instructions for the repeating units...")
echo_sheet_square = convert_slats_into_echo_commands(slat_dict=RU_slats,
                                                    ...
                                                    output_filename='{}_{}_echo.csv'.format("SW160_SW161", 'UP_repeating_units_slats_regular'))

prepare_all_standard_sheets(RU_slats, os.path.join(lab_helper_folder, '{}_{}_labhelper.xlsx'.format("SW160_SW161", 'UP_repeating_units_slats_regular')),
