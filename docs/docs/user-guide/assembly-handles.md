...
3. Click a pattern in the list to start placing it. The whole pattern follows the cursor (anchored at its top-left handle) and is placed on click. The preview turns red if any position would not land on a slat.
