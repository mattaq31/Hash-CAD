...
def convert_slats_into_echo_commands(slat_dict, destination_plate_name, output_folder, output_filename,
    ...
    for _, slat in slat_dict.items():
        for h2_num, h2 in enumerate(slat.H2_handles.values()):
            complete_list.append([slat.ID + '_h2_staple_%s' % (h2_num+1), h2['plate'], h2['well'], plate96[total_wells], transfer_volume, destination_plate_name, source_plate_type])

        for h5_num, h5 in enumerate(slat.H5_handles.values()):
            complete_list.append([slat.ID + '_h5_staple_%s' % (h5_num+1), h5['plate'], h5['well'], plate96[total_wells], transfer_volume, destination_plate_name, source_plate_type])
        total_wells += 1
