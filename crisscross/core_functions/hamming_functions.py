...
def multi_rule_hamming(slat_array, handle_array, report_score = False, report_bad_handle_sequences = False, universal_check=True, per_layer_check=False, specific_slat_groups=None, slat_length=32):
    ...
    if report_bad_handle_sequences == True:
                ...
                hallofshamehandles = [list(bag_of_slat_handles.keys())[i] for i in indices[0]]
                hallofshameantihandles = [list(bag_of_slat_antihandles.keys())[i] for i in indices[1]]
...
def generate_handle_set_and_evolve(base_array, unique_sequences=32, slat_length=32, stopat = 28, population =30, survivors=3 , percentofpointmutations = 0.0025, generations = 20, split_sequence_handles=True):
   ...
   # initiate population of handle arrays
    ...
    #handle_arries[0] = np.load('square16only2.npy')
    ...
    for ever in range(generations):
        ...
        for j in range(population):
            ...
            scores[j]= dummy['Score 1'] # all scores of the whole population
            hammings[j] = dummy['Universal'] # the hamming distances of the population
            hallofschame_Hvalues.append( dummy['Hall of shame handles']) # This is a list of bad handles for each  indiviualte
            hallofschame_AHvalues.append( dummy['Hall of shame antihandles'] )# same as above
...
#hamming_dict = multi_rule_hamming(slat_array, handle_array, report_score=True, universal_check=True)
   # ergebniss = generate_handle_set_and_evolve(base_array= slat_array)
    ...
    #slat_array, unique_slats_per_layer = generate_standard_square_slats(32)  # standard square
    #handle_array = generate_random_slat_handles(slat_array, 32)
    ergebnüsse= generate_handle_set_and_evolve(base_array= slat_array,unique_sequences= 32,  stopat = 28, population =10, survivors=3 ,percentofpointmutations = 0.03, generations=5, split_sequence_handles=True)
