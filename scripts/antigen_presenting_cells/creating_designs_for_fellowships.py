...
out_file = 'TEST'
patterning = True
base_array, x_slats, y_slats = read_design_from_excel(folder, 'slats_bottom.csv', 'slats_top.csv')
dim_size = 66
xsize = 32
ysize = 32
alpha = 0.2
...
if patterning:
    ...
    for a in range(total_antigens):
        xr = 1
        yr = 1
        while base_array[yr, xr, 0] == 0:
            xr = np.random.randint(1, 66)
            yr = np.random.randint(1, 66)
    ...
    # corner_xs = [2, 64 - int(square_side/2), 32 - int(square_side/4), 32 - int(square_side/4)]
    # corner_ys = [34 - int(square_side/4), 34 - int(square_side/4), 3, 65 - int(square_side/2)]
...
for xval in x_slats:
    if xval == 113 or xval == 114 or xval == 177 or xval == 178:
        continue
    posns = np.where(base_array[:, :, 0] == xval)
    y1, x1 = np.min(posns[0]), np.min(posns[1])
    ax.add_patch(Rectangle((x1-slatPadding, y1), xsize, 1 - 2 * slatPadding, color="red", alpha=alpha))

for yval in y_slats:
    if yval == 32 or yval == 33 or yval == 65 or yval == 66:
        continue
...
for i, j in zip(posx, posy):
    ax.add_patch(Circle((j + slatPadding, i + slatPadding), 0.35,
